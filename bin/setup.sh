...
sudo DEBIAN_FRONTEND=noninteractive apt-get install -y python3.9-venv
...
python3.9 -m venv ~/backend-songs-venv
...
source ~/backend-songs-venv/bin/activate && python3.9 -m pip install --upgrade pip wheel
